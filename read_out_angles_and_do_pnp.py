    ...
    for line in f:
      ...
      #  print(lighthouse_image_projection.dtype)
       ...
       # print(lighthouse_3d_store.dtype)
        ...
        print(np.shape(lighthouse_3d))
        #retval, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, R, T, E, F=cv.stereoCalibrate(lighthouse_3d,lighthouse_image_projection,lighthouse_image_projection_2,K,dist_coef,K,dist_coef,np.shape(lighthouse_3d),flags=flags)
        if first_run:
            _ret, rvec_est, tvec_est = cv.solvePnP(lighthouse_3d, lighthouse_image_projection, K, dist_coef,flags=cv.SOLVEPNP_ITERATIVE)
            first_run=False
        else:
           # print(np.size(tvec_est))
          #  print(rvec_est)
       ...
       # print(lighthouse_3d.T)

        #rvec, tvec = cv.solvePnPRansac(lighthouse_3d.T, lighthouse_image_projection.T, K, dist_coef)
    
       # print(_ret)
        Rmatrix, _ = cv.Rodrigues(rvec_est)

        rotated_vector= np.matmul(np.linalg.inv(Rmatrix),tvec_est)
       # print(rotated_vector)
        #print(position)
        real_vector=np.subtract(position,origin_bs1)
       # print(np.subtract(position,origin_bs1))

        X_pnp.append(rotated_vector[1])
        Y_pnp.append(-rotated_vector[0])
        Z_pnp.append(rotated_vector[2])
